...
bot.start(async (ctx) => {
    try {
        ...
        if (!isSubscribed) {
            return ctx.reply(`Botdan foydalanish uchun ${REQUIRED_CHANNEL} kanalimizga a'zo bo'ling!`, 
        ...
        ctx.reply(ctx.from.id === ADMIN_ID ? "Xush kelibsiz, Admin!" : "Xush kelibsiz! Kerakli bo'limni tanlang:", 
...
bot.action(/buy_(.+)/, async (ctx) => {
    try {
        ...
        await pool.query('UPDATE users SET balance = balance - $1, subscriptions = array_append(subscriptions, $2) WHERE id = $3', 
    ...
    try {
        ...
        if (ctx.session && ctx.from.id === ADMIN_ID) {
            ...
            if (step === 'broadcast') {
                ...
                ctx.session.name = text; ctx.session.step = 'waiting_key'; return ctx.reply("Kalit so'z (masalan: titan):");
                ...
                await pool.query('INSERT INTO series (key, name, price, link) VALUES ($1, $2, $3, $4)', 
                    [ctx.session.key, ctx.session.name, ctx.session.price, text]);
                ctx.session = null; return ctx.reply("Yangi serial qo'shildi! ✅", adminMenu);
            } else if (step === 'edit_price') {
                await pool.query('UPDATE series SET price = $1 WHERE key = $2', [parseInt(text), ctx.session.key]);
                ctx.session = null; return ctx.reply("Narx yangilandi! ✅", adminMenu);
            }
        }
    } catch (e) { console.error(e); }
});
