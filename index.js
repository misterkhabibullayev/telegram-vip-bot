    ...
    try {
        ...
        if (user.subscriptions?.length > 0) {
            const list = await Promise.all(user.subscriptions.map(async (k) => {
                ...
                return s.rows[0] ? `🎬 <a href="${s.rows[0].link}">${s.rows[0].name}</a>` : null;
            }));
        ...
        ctx.replyWithHTML(text, {
            ...
            ...Markup.inlineKeyboard([[Markup.button.callback("📜 To'lovlar tarixi", "view_history")]])
        });
    } catch (e) { console.error(e); }
});
...
bot.action(/buy_(.+)/, async (ctx) => {
    try {
        ...
        setTimeout(() => {
            bot.telegram.sendMessage(ctx.from.id, `Kecha <b>${serie.name}</b> kanalini sotib olgandingiz. Yoqdimi? Bot haqida fikringizni yozing!`, { parse_mode: "HTML" }).catch(() => {});
        }, 24 * 60 * 60 * 1000);

        bot.telegram.sendMessage(ADMIN_ID, `💰 Yangi xarid: ${serie.name}\n👤: ${ctx.from.first_name}`).catch(() => {});
    } catch (e) { console.error(e); }
});
...
bot.action("view_history", async (ctx) => {
    ...
    if (payRes.rows.length === 0) return ctx.answerCbQuery("Tarix bo'sh.", { show_alert: true });
    let text = "📜 Oxirgi to'lovlar:\n\n";
    payRes.rows.forEach(p => { text += `🔹 ${new Date(p.created_at).toLocaleDateString()} — ${p.amount} so'm\n`; });
    ctx.reply(text);
});
